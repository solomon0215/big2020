...
//					File file = new File("C:\\big2020\\test.dat");
					...
					/*
					 * json = JsonParser.parseWrsJson(service_url, service_key,
					 * String.valueOf(pageNo), args[0], args[1], args[2], args[3]);
					 */
					...
					// 2020.06.05 : String 리턴으로 잡았더니 에러 남.. JSONObject리턴으로 수정하고, 해당 메서드에 빈 json 로직을
					// 넣음
					/*
					 * if(json.indexOf("</") > -1){ System.out.print("공공데이터 서버 비 JSON 응답, stDt :" +
					 * args[0] + ": stTm :" + args[1] + ": edDt :" + args[2] + ": edTm :" +
					 * args[3]); json
					 * ="{\"response\":{\"header\":{\"resultCode\":\"00\",\"resultMsg\":\"NORMAL SERVICE.\"},\"body\":{\"items\":\"\",\"numOfRows\":10,\"pageNo\":1,\"totalCount\":0}}}"
					 * ; }
					 */

					JSONObject count_obj = JsonParser.parseWrsJson_obj(service_url, service_key, String.valueOf(pageNo),
							args[0], args[1], args[2], args[3]);

					// 빈값 JSON 및 서버 비응답 재처리
					String chkRsCode = ((JSONObject) ((JSONObject) count_obj.get("response")).get("header"))
							.get("resultCode").toString().trim();
					JSONObject chkBody = (JSONObject) ((JSONObject) count_obj.get("response")).get("body");
					if ((chkRsCode.equals("03"))) {
						System.out.println("비정상 재처리 시작");
						Thread.sleep(3000);
						count_obj = JsonParser.parseWrsJson_obj(service_url, service_key, String.valueOf(pageNo),
								args[0], args[1], args[2], args[3]);
					...
					for (int i = 1; i <= pageCount; i++) {
						...
						/*
						 * json = JsonParser.parseWrsJson(service_url, service_key, String.valueOf(i),
						 * args[0], args[1], args[2], args[3]);
						 */
						 ...
						 * 분기
						 */
						JSONObject obj = JsonParser.parseWrsJson_obj(service_url, service_key, String.valueOf(i),
								args[0], args[1], args[2], args[3]);
						...
						if ((!(resultCode.equals("00")) && !(resultCode.equals("03")))) {
							...
							if (items.get("item") instanceof JSONObject) {
								...
								while (iter.hasNext()) {
									...
									occrrncDt = JsonParser.colWrite_String(occrrncDt, keyname, "occrrncDt",
											items_jsonObject);
									fcltyNm = JsonParser.colWrite_String(fcltyNm, keyname, "fcltyNm", items_jsonObject);
									fcltyMngNo = JsonParser.colWrite_String(fcltyMngNo, keyname, "fcltyMngNo",
											items_jsonObject);
									dataVal = JsonParser.colWrite_String(dataVal, keyname, "dataVal", items_jsonObject);
									itemUnit = JsonParser.colWrite_String(itemUnit, keyname, "itemUnit",
											items_jsonObject);
									dataItemDesc = JsonParser.colWrite_String(dataItemDesc, keyname, "dataItemDesc",
											items_jsonObject);
									dataItemTagsn = JsonParser.colWrite_String(dataItemTagsn, keyname, "dataItemTagsn",
											items_jsonObject);
									dataItemDiv = JsonParser.colWrite_String(dataItemDiv, keyname, "dataItemDiv",
											items_jsonObject);
								...
								for (int r = 0; r < items_jsonArray.size(); r++) {
									...
									while (iter.hasNext()) {
										...
										dataItemDesc = JsonParser.colWrite_String(dataItemDesc, keyname, "dataItemDesc",
												item_obj);
										dataItemTagsn = JsonParser.colWrite_String(dataItemTagsn, keyname,
												"dataItemTagsn", item_obj);
										dataItemDiv = JsonParser.colWrite_String(dataItemDiv, keyname, "dataItemDiv",
												item_obj);
			...
			e.printStackTrace();
			System.out.println("클래스명 : " + Thread.currentThread().getStackTrace()[1].getClassName() + ", stDt :"
					+ args[0] + ": stTm :" + args[1] + ": edDt :" + args[2] + ": edTm :" + args[3]);
			System.exit(-1);
		}
